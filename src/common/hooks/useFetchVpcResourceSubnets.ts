...
export const useFetchVpcResourceSubnets = (provider: string, region: string, id: string, accountId: string) => {
  ...
  const fetchForProvider = (prov: string): Promise<any[]> => {
    return new Promise((resolve, reject) => {
      const request = new ListSubnetsRequest();
      request.setProvider(prov);
      request.setVpcId(id);
      request.setRegion(region);
      request.setAccountId(accountId);
      infraSdkResourcesClient.listSubnets(request, {}, (err: any, response: any) => {
        if (err) return reject(err);
        const data = response?.getSubnetsList();
        if (data) {
          const result = data.map((subnet: any) => {
  ...
  const fetchVpcResourcesSubnets = async () => {
    try {
      let results: any[] = [];
